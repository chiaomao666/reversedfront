    ...
    const observer = new MutationObserver(mutations => {
        ...
        for (const m of mutations) {
            ...
            for (const node of m.removedNodes) {
                if (node.nodeType !== 1) continue;
                if (hasPrefix(node, ROOT_PREFIX)) rootRemoved = true;
            }
        }

        // 根容器被整個祖先帶走時不會出現在 removedNodes，靠 isConnected 兜底
        if (battleRoot && !battleRoot.isConnected) {
            battleRoot = null;
            rootRemoved = true;
        }

        if (rootRemoved) {          // 戰鬥畫面關閉，收掉常駐顯示
            cancelCapture();
            removePersisted();
        }
        if (rootAdded) {            // 新一場戰鬥開始，先清掉上一場殘留的
            removePersisted();
        }

        // 不在戰鬥畫面就到此為止：不排程、不查詢，成本只有上面的迴圈
        if (!sawElement || !battleRoot) return;
        scheduleCapture();
    });
